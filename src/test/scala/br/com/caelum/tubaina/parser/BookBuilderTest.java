...
public class BookBuilderTest {
	...
	public void testBuildSimpleBook() {
		...
		String chap1 = "[chapter     O que é java?   ]\n" + "texto da seção\n"
		+ "[section Primeira seção]\n" + "texto da prim seção\n" + "[section Segunda seção]\n"
		+ "texto da segunda seção\n\n";
		String chap2 = "[chapter Introdução]\n" + "Algum texto de introdução\n";
		
		Book book = new TubainaParser("livro").generate(chap1, chap2);
	...
	private List<Chapter> getChapters(final String string) {
		return new TubainaParser("livro").generate(string).getChapters();
	}
